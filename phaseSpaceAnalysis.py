...
centerZRBF=zArrFine[np.argmax(voigtFitArrFine)]

#find fwhm from fit
maximum=np.max(voigtFitArrFine)

zLeftArg=np.argwhere(voigtFitArrFine>(maximum+np.min(voigtFitArrFine))/2)[0]
zRightArg=np.argwhere(voigtFitArrFine>(maximum+np.min(voigtFitArrFine))/2)[-1]
...
#----RBF fit-----
smoothFact=1e0#you might need to play with the smooth value. Change by factors of 10
...
plt.plot(zArr[yStart:yEnd], TGammaLockedList, label='gamma=5.87MHz')
plt.plot(zArr[yStart:yEnd], TGammaFreeList, label='gamma>=5.87MHz')
